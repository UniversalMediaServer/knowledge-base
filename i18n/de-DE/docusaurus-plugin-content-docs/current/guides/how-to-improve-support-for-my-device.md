...
- Die Möglichkeiten dieses Renderers zu definieren

Wir haben eine Standard-Renderer-Konfigurationsdatei, die eine Dokumentation zu allen unseren Renderer-Einstellungen enthält. Sehen Sie sich die neueste Version an auf https://github.com/UniversalMediaServer/UniversalMediaServer/blob/master/src/main/external-resources/renderers/DefaultRenderer.conf
...
1. Gehen Sie zum `Logs` - Tab in UMS suchen Sie nach dem Text `"Media - Renderer wurde nicht erkannt." Mögliche HTTP-Header zur Identifikation:`. Diese Information ist das, was nötig ist, damit UMS Ihr Gerät erkennen kann.

1. Suchen Sie in Ihrer neuen Konfigurationsdatei nach der Zeile, welche `UserAgentSearch` und/oder `UpnpDetailsSearch` definiert und ersetzen Sie die Werte durch diese Identifikationsinformationen.
...
1. Um sicherzustellen, dass die Transkodierung auf Ihrem Gerät funktioniert, spielen Sie eine Datei aus dem Ordner </code>#--UMWANDLUNG--#</0> ab. Spielen Sie in diesem Ordner einen der `FFmpeg`  - Einträge ab. Wenn er abgespielt wird, funktioniert die Transkodierung/Umwandlung.
...
1. Zusätzlich können Sie sich andere Renderer-Konfigurationen anschauen, die sich im Ordner "renderers" des Installationsverzeichnisses befinden, um zu sehen, was sie bewirken. Manchmal brauchen Sie Hilfe, die wir Ihnen in unserem Forum geben können. Und denken Sie bitte daran, uns die erreichten Verbesserungen mitzuteilen, damit andere Benutzer mit Ihrem Gerät von der Lösung profitieren könnnen.  Wir werden Sie in unserer Release-Ankündigung und der Änderungshistorie lobend erwähnen.
